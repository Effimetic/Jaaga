...
import { Boat, Destination, Schedule } from '../types';
...
export const OwnerSearchScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  React.useEffect(() => {
    loadData();
  }, [dateFilter, customStartDate, customEndDate, loadData]);
    ...
    <Card key={schedule.id} variant="outlined" padding="md" style={{ marginBottom: 12 }}>
      <View style={{ gap: 12 }}>
        ...
        <View style={{ flexDirection: 'row', gap: 8 }}>
          ...
          <TouchableOpacity
            onPress={() => navigation.navigate('BookingFlow', { scheduleId: schedule.id })}
            style={{
              flex: 1,
          ...
          <View style={{ marginBottom: 16 }}>
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
              <TouchableOpacity
                ...
                style={{
                  backgroundColor: selectedDestination === '' ? '#18181b' : '#f3f4f6',
                  paddingHorizontal: 12,
                  paddingVertical: 6,
                  borderRadius: 16,
                  borderWidth: 1,
                  borderColor: selectedDestination === '' ? '#18181b' : '#e5e7eb'
                }}
              >
                <Text style={{
                  fontSize: 12,
                  fontWeight: '500',
                  color: selectedDestination === '' ? '#ffffff' : '#374151'
                }}>
                  All Destinations
                </Text>
              </TouchableOpacity>
              {destinations.map((destination) => (
                <TouchableOpacity
                  ...
                  style={{
                    backgroundColor: selectedDestination === destination.id ? '#18181b' : '#f3f4f6',
                    paddingHorizontal: 12,
                    paddingVertical: 6,
                    borderRadius: 16,
                    borderWidth: 1,
                    borderColor: selectedDestination === destination.id ? '#18181b' : '#e5e7eb'
                  }}
                >
                  <Text style={{
                    fontSize: 12,
                    fontWeight: '500',
                    color: selectedDestination === destination.id ? '#ffffff' : '#374151'
                  }}>
                    {destination.name}
                  </Text>
                </TouchableOpacity>
          ...
          {schedules.length === 0 ? (
            <View style={{ padding: 32, alignItems: 'center' }}>
              ...
              <Text style={{ fontSize: 16, color: '#6b7280', marginTop: 16, textAlign: 'center' }}>
                No schedules found for selected destination and date range
              </Text>
              <Text style={{ fontSize: 14, color: '#9ca3af', marginTop: 8, textAlign: 'center' }}>
                Try changing the destination filter or create a new schedule
              </Text>
              <TouchableOpacity
                onPress={() => navigation.navigate('ScheduleWizard')}
                style={{
      ...
      return (
        <View>
          ...
          <View style={{ marginBottom: 16 }}>
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
              <TouchableOpacity
                onPress={() => setSelectedBoat('')}
                style={{
                  backgroundColor: selectedBoat === '' ? '#18181b' : '#f3f4f6',
                  paddingHorizontal: 12,
                  paddingVertical: 6,
                  borderRadius: 16,
                  borderWidth: 1,
                  borderColor: selectedBoat === '' ? '#18181b' : '#e5e7eb'
                }}
              >
                <Text style={{
                  fontSize: 12,
                  fontWeight: '500',
                  color: selectedBoat === '' ? '#ffffff' : '#374151'
                }}>
                  All Boats
                </Text>
              </TouchableOpacity>
              {boats.map((boat) => (
                <TouchableOpacity
                  ...
                  style={{
                    backgroundColor: selectedBoat === boat.id ? '#18181b' : '#f3f4f6',
                    paddingHorizontal: 12,
                    paddingVertical: 6,
                    borderRadius: 16,
                    borderWidth: 1,
                    borderColor: selectedBoat === boat.id ? '#18181b' : '#e5e7eb'
                  }}
                >
                  <Text style={{
                    fontSize: 12,
                    fontWeight: '500',
                    color: selectedBoat === boat.id ? '#ffffff' : '#374151'
                  }}>
                    {boat.name}
                  </Text>
                </TouchableOpacity>
          ...
          {schedules.length === 0 ? (
            <View style={{ padding: 32, alignItems: 'center' }}>
              <MaterialCommunityIcons name="ferry" size={48} color="#d1d5db" />
              <Text style={{ fontSize: 16, color: '#6b7280', marginTop: 16, textAlign: 'center' }}>
                No schedules found for selected boat and date range
              </Text>
              <Text style={{ fontSize: 14, color: '#9ca3af', marginTop: 8, textAlign: 'center' }}>
                Try changing the boat filter or create a new schedule
              </Text>
              <TouchableOpacity
                onPress={() => navigation.navigate('ScheduleWizard')}
                style={{
